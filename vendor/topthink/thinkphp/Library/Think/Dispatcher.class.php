...
class Dispatcher {
    ...
    static public function dispatch() {
        ...
        // 获取模块名称
        define('MODULE_NAME', strtolower(defined('BIND_MODULE')? BIND_MODULE : self::getModule($varModule)));

        // 检测模块是否存在
        if( MODULE_NAME && (defined('BIND_MODULE') || !in_array_case(MODULE_NAME,C('MODULE_DENY_LIST')) ) && (is_dir(APP_PATH.MODULE_NAME) || is_dir(COMPONENT_PATH . MODULE_NAME) || is_dir(PLAYGROUND_PATH . MODULE_NAME))){
            // 定义当前模块路径
            if(is_dir(PLAYGROUND_PATH . MODULE_NAME)){
                define('MODULE_PATH', PLAYGROUND_PATH . MODULE_NAME . '/');
            } else if (is_dir(COMPONENT_PATH . MODULE_NAME)) {
                define('MODULE_PATH', COMPONENT_PATH . MODULE_NAME . '/');
            } else {
            ...
            // 加载模块配置文件
            if(is_file(MODULE_PATH.'config/config'.CONF_EXT))
                C(load_config(MODULE_PATH.'config/config'.CONF_EXT));
            // 加载应用模式对应的配置文件
            if('common' != APP_MODE && is_file(MODULE_PATH.'config/config_'.APP_MODE.CONF_EXT))
                C(load_config(MODULE_PATH.'config/config_'.APP_MODE.CONF_EXT));
            // 当前应用状态对应的配置文件
            if(APP_STATUS && is_file(MODULE_PATH.'config/'.APP_STATUS.CONF_EXT))
                C(load_config(MODULE_PATH.'config/'.APP_STATUS.CONF_EXT));

            // 加载模块别名定义
            if(is_file(MODULE_PATH.'config/alias.php'))
                Think::addMap(include MODULE_PATH.'config/alias.php');
            // 加载模块tags文件定义
            if(is_file(MODULE_PATH.'config/tags.php'))
                Hook::import(include MODULE_PATH.'config/tags.php');
            // 加载模块函数文件
            if(is_file(MODULE_PATH.'helpers/function_helper.php'))
                include MODULE_PATH.'helpers/function_helper.php';
