	...
	$.fn.qqFace = function(options){
		...
		$(this).click(function(e){
			...
			if($('#'+id).length<=0){
				...
				for(var i=1; i<=71; i++){
					labFace = '['+tip+i+']';
					strFace += '<td style="text-align:center"><img src="'+path+i+'.gif" onclick="$(\'#'+option.assign+'\').setCaret();$(\'#'+option.assign+'\').insertAtCaret(\'' + labFace + '\');" /></td>';
...
function replace_em(str) {
	...
	str = str.replace(/\>/g, '&gt;');
	str = str.replace(/\n/g, '<br/>');
	str = str.replace(/\[em_([0-9]*)\]/g, "<img src='"+ window.ROOT_URL +"/public/vendor/layui/images/face/$1\.gif' />");
	return str;
}