var dscmallKefu = {
    ...
    init : function(){
        ...
        if(this.listen_route == '' || this.port == ''){
            layer.msg('socket 端口未设置');
            return;
        }
        this.inited = true;
        dscmallKefu.come_form = this.IsPC() ? 'PC' : 'H5';
        dscmallKefu.IsSsl();
        this.connect();
        dscmallEvent.init();
    },
    connect : function(){
        ...
        this.socket.onopen = function() {
            that.socket.send(that.json_decode({uid:that.user.user_id, name:that.user.user_name, type:'login', user_type:that.user.user_type, avatar:that.user.avatar, store_id: that.user.store_id,  origin: (that.IsPC()) ? 'PC' : 'H5'}));
        };
    ...
    sendmsg : function(){
        ...
        this.socket.send(JSON.stringify(this.message));
        dscmallEvent.add_message({message:this.message.msg, name:dscmallKefu.user.user_name, time: dscmallKefu.SystemDate(), origin:dscmallKefu.come_form }, 1);
    },
    //
