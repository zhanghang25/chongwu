...
function get_drp($user_id=0, $is_drp=0)
{
    ...
    $info = [];
    if ($shopInfo['shop_portrait']) {
        $info['headimgurl'] = get_image_path($shopInfo['shop_portrait']);
    } else {
       //用户名、头像
        $user_nick = get_user_default($user_id);
        $info['nickname'] = $user_nick['nick_name'];
        $info['headimgurl'] = $user_nick['user_picture'];
    }
    // 店铺信息
...
function drp_audit_status($user_id=0)
{
    ...
    $drp_shop= dao('drp_shop')->field("audit,status")->where(["user_id"=>$user_id])->find();
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0) {
        ...
        foreach ($res as $k => $row) {
            if ($row['is_show']) {
                ...
                $three_arr[$k]['url'] = url('drp/user/drpgoodslist/', ['id'=>$row['cat_id']]);
                $three_arr[$k]['shop_url'] = url('drp/shop/index/', ['cat_id'=>$row['cat_id'],'id'=>$shop_id]);
...
{
    $time = gmtime();
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('drp_transfer_log') .
            ' (user_id, money,add_time)' .
...
function drp_rank_info($user_id = 0)
{
    ...
    $drp_info = dao('drp_shop')->field('credit_id')->where(['user_id'=>$user_id])->find();
    if ($drp_info['credit_id'] > 0) {
        $rank_info = dao('drp_user_credit')->field('id,credit_name,min_money,max_money')->where(['id'=>$drp_info['credit_id']])->find();
    } else {
        //统计分销商所属订单金额
        $sql = "SELECT sum(money) as totals FROM " . $GLOBALS['ecs']->table('order_goods') . " o" .
                        " LEFT JOIN " . $GLOBALS['ecs']->table('drp_affiliate_log') . " a ON o.order_id = a.order_id" .
                        " WHERE a.separate_type != -1 and a.user_id = " . $user_id;
 ...
 * @return
 */
function get_qrcode_config($front = false)
{
    $config = dao('drp_config')->where(['code' => 'qrcode'])->getField('value');
    ...
    if ($front == true) {
        ...
        $config['avatar'] = isset($config['avatar']) ? $config['avatar'] : 1;
    }
