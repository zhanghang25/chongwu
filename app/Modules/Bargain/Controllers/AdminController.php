...
class AdminController extends BackendController
{
    ...
    public function actionAddgoods()
    {
        ...
        if (IS_POST) {
            ...
            $id = I('id', '', 'intval');
            $target_price = I('target_price', '', '');//目标价格
            $product_id = I('product_id', '', '');//货品ID
            $activity_goods_attr = I('bargain_id', '', '');//活动商品属性列表id
        ...
        if (I('id')) {
            $id = I('id', '', 'intval');
            $info = $this->model->table('bargain_goods')->where(['id' => $id])->find();//砍价商品信息
    ...
    public function actionBargainlog()
    {
        ...
        foreach ($list as $key => $val) {
            ...
            //$list[$key]['user_name'] = encrypt_username($user_nick['nick_name']);
