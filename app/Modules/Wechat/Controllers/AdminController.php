        ...
        if (IS_POST) {
            ...
            if ($data['type'] == 0) {
                if (!empty($data['expire_seconds']) && $data['expire_seconds'] > 2592000) {
                    exit(json_encode(array('status' => 0, 'msg' => '临时二维码最大不能超过30天')));
                }
                if ($data['scene_id'] < 100001 || $data['scene_id'] > 4294967295) {
                    exit(json_encode(array('status' => 0, 'msg' => '临时二维码取值区间 100001 - 4294967295')));
                }
            }
            // 永久二维码
            if ($data['type'] == 1) {
                if ($data['scene_id'] < 1 || $data['scene_id'] > 100000) {
                    exit(json_encode(array('status' => 0, 'msg' => '永久二维码取值区间 1- 100000')));
                }
            }
    ...
    public function actionVoice()
    {
        ...
        if (IS_POST) {
            if ($_FILES['voice']['name']) {
                $type = array('audio/amr', 'audio/x-mpeg', 'audio/mp3'); // amr,mp3
        ...
        if (IS_AJAX) {
            ...
            if (!empty($id)) {
                ...
                if (!empty($template['template_id'])) {
                    ...
                    dao('wechat_template')->data(array('template_id' => '', 'status' => 0))->where(array('id' => $id))->save();
