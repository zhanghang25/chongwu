...
class AdminController extends BackendController
{
    ...
    public function actionAddgoods()
    {
        ...
        if (I('id')) {
            ...
            $goods = $this->model->table('goods')->field('goods_name,user_id')->where(array('goods_id' => $info['goods_id']))->find();//商品
        ...
        //频道列表
        //$this->assign('team_list', team_cat_list(0, $info['tc_id']));//拼团频道树形
    ...
    public function actionTeaminfo()
    {
        ...
        switch ($status) {
            case '1':
                ...
                break;
            case '2':
                $where .= " and tl.status < 1 and '" . gmtime() . "'< (tl.start_time+(tg.validity_time*3600)) and tg.is_team = 1 ";//拼团中
                break;
            case '3':
                $where .= " and tl.status = 1 ";//成功团
                break;
            case '4':
                $where .= " and tl.status != 1 and ('" . gmtime() . "'> (tl.start_time+(tg.validity_time*3600)) || tg.is_team != 1)";//失败团
                break;
